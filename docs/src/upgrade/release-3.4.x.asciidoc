...
The `ConnectionPool` now also has a min and max size. A new connection is only created if all existing connections reached their `MaxInProcessPerConnection` limit. If additionally
the max size of the `ConnectionPool` is reached, then a `NoConnectionAvailableException` is thrown which makes this a breaking change.
